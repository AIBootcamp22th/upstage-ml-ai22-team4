...
@st.cache_data
def load_data():
    # Load 10% sample for speed in presentation if file is huge, but here we load full for map accuracy or limit logic
    # Using 'nrows' to keep it responsive if needed, but 'train.csv' is ~1GB? No, ~1M rows.
    # Let's load full but cache it carefully. It might take 10s.
...
data['거래월'] = data['계약년월'].str[4:].astype(int)
    ...
    **✅ 해결책: K-Means Geo Clustering**
    ...
    # Load Transport Data for Visualization
    try:
        bus_df = pd.read_csv('../data/bus_feature.csv')
        sub_df = pd.read_csv('../data/subway_feature.csv')
